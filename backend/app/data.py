...
def build_content_analysis_response(mode: str, user_input: str) -> dict:
    ...
    if mode == "url" and analyzed_text:
        try:
            source_title, source_excerpt, _content_type = _fetch_url_preview(analyzed_text)
            fetched = True
        except (HTTPError, URLError, TimeoutError, ValueError, UnicodeError, OSError):
            source_excerpt = ""

    analysis_basis = " ".join(part for part in (source_title, source_excerpt, analyzed_text) if part)
    score = _score_content(analysis_basis, mode == "url", fetched)
    bias = _bias_label(score)
    emotion = _emotion_label(analysis_basis)
    reliability = _reliability_label(score)
    fact_accuracy = f"{min(99, score + 6)}%"

    summary_parts = []
    if mode == "url":
        summary_parts.append(f"URL analyzed: {analyzed_text}")
        if source_title:
            summary_parts.append(f"Page title: {source_title}")
        if source_excerpt:
            summary_parts.append(f"Page preview: {source_excerpt}")
        if not fetched:
            summary_parts.append("Live preview unavailable, so the score was based on the URL and reachable metadata only.")
    else:
        summary_parts.append(f"Text analyzed: {analyzed_text}")
        summary_parts.append(f"Tone estimate: {emotion}; bias estimate: {bias}; reliability estimate: {reliability}.")
        if source_excerpt:
            summary_parts.append(f"Evidence preview: {source_excerpt}")

    if not summary_parts:
        summary_parts.append("No input provided for analysis.")
    ...
    return {
        ...
        "chips": _build_chips(mode, fetched, score, source_title, source_excerpt),
        "metrics": [
        ...
        "recommendations": _build_recommendations(mode, score, fetched, source_title, source_excerpt),
